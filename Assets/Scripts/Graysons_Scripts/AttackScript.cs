        ...
        if (attackCountdownTimer <= 0)
        {
            ...
            if (Input.GetKey(KeyCode.Space))
            {
                ...
                for (int i = 0; i < enemiesToDamage.Length; i++)
                {
                    ...
                    GraysonEnemyScript enemyScript = enemiesToDamage[i].GetComponent<GraysonEnemyScript>();
                    // If there is an enemy script
                    if (enemyScript)
                    {
                        // Damage
                        enemiesToDamage[i].GetComponent<GraysonEnemyScript>().health -= damage;
                        // Print health levels
                        Debug.Log(enemiesToDamage[i].GetComponent<GraysonEnemyScript>().health);
